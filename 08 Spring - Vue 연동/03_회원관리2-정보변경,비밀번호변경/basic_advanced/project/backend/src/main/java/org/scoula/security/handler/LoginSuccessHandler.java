...
public class LoginSuccessHandler implements AuthenticationSuccessHandler {
    ...
    // JsonResponse로 보낼 값을 만들어야 함.
    // 성공 -> uthentication 객체가 이미 만들어져서 SecurityContextHolder에 들어가 있음.
    private AuthResultDTO makeAuthResult(CustomUser user) {
        String username = user.getUsername();
        // 토큰 생성
        String token = jwtProcessor.generateToken(username);
        // 토큰 + 사용자 기본 정보 (사용자명, ...)를 묶어서 AuthResultDTO 구성
        // 'user.getMember()' 값이 'UserInfoDTO'에 저장됨
        return new AuthResultDTO(token, UserInfoDTO.of(user.getMember()));
    }

    @Override
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
        // authentication.getPrincipal(): 인증 결과 Principal
