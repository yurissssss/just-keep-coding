...
// 현재 라우트의 정보(쿼리스트링 등)를 얻기 위해 useRoute 훅 가져오기
...
// 로그인 함수가 호출되면 전달된 파라미터를 받아서 해당 주소로 라우팅
// 인증가드가 login을 호출하면서 다음 라우팅될 주소를 파라미터로 전달
const login = async () => {
  console.log(member);
  try {
    ...
    if (cr.query.next) {
      // cr.query.next: 다음 주소가 들어있음
      // http://localhost:5173/auth/login?next=board/create
